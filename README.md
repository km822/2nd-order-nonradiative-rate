...
The underlying formulation is known as thermal vibration correlation function (TVCF) formulation and derived analytically assuming harmonic nuclear modes. This implementation takes it further by analytically removing the inherent singularity issues associated with TVCFs, hence achieving stable and robust numerical implementation. For the detailed workflow of the derivations, we refer readers to the PDF files in the 'derivation' directory.
...
To generate a series of geometry files for normal mode displacements, run
```
